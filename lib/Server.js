...
class Server {
    ...
    if (this.options.webSocketServer) {
      ...
      compilers.forEach((compiler) => {
        ...
        if (this.options.hot) {
          ...
          // webpack-dev-server 之后，如果 hot 为 true，则不需要在 webpack.config.js 文件中重复配置 HotModuleReplacementPlugin 插件了
          if (HMRPluginExists) {
            this.logger.warn(
  ...
  /**
   * @description
   *  setupHooks主要做的就是在webpack的done钩子上挂了个给客户端广播消息的回调，
   *  通过这个回调，客户端就能知道项目工程代码有更新，
   *  这时候客户端就会发请求给express服务去获取最新的webpack打包的代码
   * @private
   * @returns {void}
   */
  setupHooks() {
  ...
  /**
   * @description
   * （1）setupDevMiddleware 函数返回结果是 express 标准的 middleware 用于处理浏览器静态资源的请求。
   * 执行过程中显示初始化了一个 context 对象，
   * 默认非 lazy 模式，开启了 webpack 的 watch 模式开始启动编译。
   * （2）然后将 compiler 的原来基于 fs 模块的 outputFileSystem 替换成 memory-fs模块的实例。
   * memory-fs 是实现了 node的 fs api 的基于内存的 fileSystem，这意味着 webpack 编译后的资源不会被输出到硬盘而是内存。
   * 最后将真正处理请求的 middleware 返回装载在 express 上。（根据patch-package打的补丁找到相关代码处）
   * @private
   * @returns {void}
   */
  setupDevMiddleware() {
  ...
  createServer() {
    ...
    // 默认通过 http.createServer 创建 this.server，文档地址：https://nodejs.org/dist/latest-v18.x/docs/api/http.html#httpcreateserveroptions-requestlistener
    // eslint-disable-next-line import/no-dynamic-require
  ...
  createWebSocketServer() {
    ...
    // 如果是调用 ws 模块的话，这个implementation是WebSocket.Server
  ...
  sendStats(clients, stats, force) {
    ...
    if (
      ...
      // 浏览器中的client代码中会创建WebSocketClient示例，这里发送 ok 之后，注入bundle.js的代码就会被触发，进而执行 reloadApp 函数
  ...
  async start() {
    ...
    // 这里创建完 this.server 并启动成功之后，后续调用 createWebSocketServer 创建 ws 服务
    await /** @type {Promise<void>} */ (
      new Promise((resolve) => {
        /** @type {import("http").Server} */
